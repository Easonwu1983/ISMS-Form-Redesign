(function () {
  window.createShellModule = function createShellModule(deps) {
    ...
    function renderSidebar() {
      ...
      if (!sidebarEl) return;
      sidebarEl.innerHTML = '<div class="sidebar-logo" style="height:56px;max-height:56px;min-height:0;overflow:hidden;display:flex;align-items:center;padding:8px 14px;gap:10px"><span class="sidebar-brand-icon" style="flex-shrink:0;display:flex;align-items:center">' + ntuLogo('ntu-logo-sm') + '</span><div class="sidebar-brand-text" style="min-width:0;overflow:hidden"><h1 style="font-size:0.8rem;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin:0;line-height:1.2">內部稽核管考追蹤系統</h1><p style="font-size:0.6rem;margin:2px 0 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#95a7bc;letter-spacing:0.08em">ISMS 管考與追蹤平台</p></div></div><nav class="sidebar-nav">' + nav + '</nav><div class="sidebar-footer"><div class="sidebar-footer-user"><span class="sidebar-footer-name">' + esc(u.name) + '</span><span class="badge-role ' + getRoleBadgeClass(u.role) + '">' + getRoleLabel(u.role) + '</span></div><button class="sidebar-logout-btn" data-action="shell.logout"><span class="nav-icon">' + ic('log-out') + '</span>登出系統</button>' + renderVersionChip('sidebar-version-chip') + '</div>';
