...
(function () {
  ...
  // 引用共用白名單（與 shared/unit-categories.js 同步）
  const UNIT_ADMIN_PRIMARY_WHITELIST = new Set([
    '秘書室', '教務處', '學生事務處', '總務處', '研究發展處',
    '國際事務處', '財務管理處', '法務處', '稽核室', '主計室', '人事室',
    '圖書館', '計算機及資訊網路中心', '出版中心', '環境保護暨職業安全衛生中心',
    '研究誠信辦公室', '永續辦公室', '校務研究辦公室', '學校分部總辦事處',
    '藝文中心', '校園規劃小組', '校友中心', '保健中心', '校園安全中心',
    '臺大醫院', '臺大醫院環境及職業醫學部', '臺大癌醫中心醫院'
  ]);
  ...
  let appFeatureRuntimeModuleApi = null;
  function getAppFeatureRuntimeModule() {
    if (appFeatureRuntimeModuleApi) return appFeatureRuntimeModuleApi;
    if (typeof window === 'undefined' || typeof window.createAppFeatureRuntimeModule !== 'function') {
      throw new Error('app-feature-runtime-module.js not loaded');
    }
    appFeatureRuntimeModuleApi = window.createAppFeatureRuntimeModule();
    window._appFeatureRuntimeModule = appFeatureRuntimeModuleApi;
    return appFeatureRuntimeModuleApi;
  }
  const appFeatureRuntime = getAppFeatureRuntimeModule().createAccess({
    getAdminModuleConfig: function () {
      return {
        ROLES,
        ROLE_BADGE,
        currentUser,
  ...
  const DEFAULT_CHECKLIST_SECTIONS = [
    {
      section: '1. 資安意識及資安通識教育訓練', items: [
        ...
        { id: '1.2', text: '單位同仁是否每年接受 3 小時以上之資通安全通識教育？', hint: '115 年資通安全教育訓練執行情形-簽核表、全校資安通識教育訓練時數-統計表' },
    ...
    },
    {
      section: '2. 資通系統盤點暨安全等級評估', items: [
        ...
        { id: '2.5', text: '資訊資產上線前是否進行系統更新與漏洞修補，並採取適當管控機制，如連線控管、變更廠商預設帳密、禁止使用弱密碼？', hint: '系統更新與漏洞修補紀錄、弱點掃描報告、變更預設帳密的操作紀錄或畫面截圖、密碼強度設定畫面截圖、禁用弱密碼或預設帳號的政策文件、防火牆規則或存取控制清單（ACL）' }
      ]
    },
    {
      section: '3. 資訊資產盤點暨風險評鑑', items: [
        ...
        { id: '3.3', text: '危害國家資通安全產品是否已清查列冊管理？', hint: '請參考資通盤點系統。若有專案使用者，已採取限制特定區域使用與定期確認替代方案之措施' },
        { id: '3.4', text: '是否清查物聯網設備，盤點範圍包含單位採購、公務使用之物聯網設備，並納入「資訊資產清冊」列管？', hint: '請參考資通盤點系統' },
        { id: '3.5', text: '資訊資產安裝完畢後是否立即更新廠商所預設之通行碼？', hint: '變更預設通行碼的操作紀錄（含操作人員、變更時間）、系統管理介面截圖（帳號最後修改時間或密碼設定畫面）、資訊資產驗收或上線檢查清單（包含「變更預設密碼」項目並有簽核）' },
        { id: '3.6', text: '是否規劃已停止支援服務（EOS）資訊資產的汰換及升級計畫？', hint: 'EOS 資產清冊、汰換與升級計畫文件' }
      ]
    },
    {
      section: '4. 日常作業資訊安全管理', items: [
        { id: '4.1', text: '汰除之儲存設備是否已確認機敏資訊已刪除？並依本校「資訊設備回收再使用及汰除之安全控制作業程序與校內報廢程序」辦理？', hint: '儲存設備資料清除紀錄（設備編號、清除方式如覆寫/消磁/摧毀、執行人員與日期）、資料刪除或摧毀證明文件（如 Blancco、DBAN 等工具報告或物理破壞紀錄照片）、校內報廢核准單或簽核文件、資產報廢清單或總表、環保回收或委外處理紀錄' },
        { id: '4.2', text: '【適用設有機房單位】是否針對電腦機房及重要區域之安全控制、人員進出管控、環境維護（如溫溼度控制）等項目建立適當之管理措施，落實執行？', hint: '機房/重要區域出入紀錄（門禁系統紀錄、刷卡紀錄、訪客登記表）、人員進出管控規範文件、機房環境監控紀錄（溫溼度紀錄表、自動監控系統截圖、異常通知記錄）、CCTV 架設位置圖與運作紀錄、機房設備保養與環境維護紀錄（冷氣維修、UPS 巡檢、消防設備檢測）' },
        { id: '4.3', text: '【適用設有機房單位】針對電腦機房及重要區域之公用服務（如水、電、消防及通訊等）建立適當之備援方案？', hint: 'UPS/發電機設備清單與配置圖、電力異常應變 SOP（跳電處理流程、自動切換機制、通知流程）、消防系統說明文件（氣體滅火設備、煙霧/溫度感測器配置圖與年度檢測報告）、定期演練紀錄與照片（模擬電力中斷、火災警報啟動、通訊中斷應變演練）' }
      ]
    },
    {
      section: '5. 資通系統發展及維護安全', items: [
        { id: '5.1', text: '是否定期執行重要資料之備份作業？', hint: '備份排程設定截圖或設定文件（每日/每週等備份路徑與方式）、備份作業紀錄或日誌（Log）、備份政策或作業流程文件（備份週期、保留期限、異地/雲端備援策略）、備份資料驗證紀錄（定期還原測試結果或備份完整性檢查報告）' },
    ...
    },
    {
      section: '6. 資通系統或服務委外辦理之管理', items: [
        { id: '6.1', text: '辦理資通系統建置、軟體開發、維運服務或資安強化等採購案時，是否已於採購規劃階段完成資通安全需求識別，並將資通安全責任分工、資料保護機制、存取控制原則、資安事件通報與應變機制、履約監督與稽核方式，以及契約終止後之資料返還、移交、刪除或銷毀機制，完整納入招標文件或需求規格中？', hint: '招標文件或需求規格書（含資安需求條款）' },
        { id: '6.2', text: '選任受託廠商時，是否已將資通安全能力納入廠商評選或資格審查項目，並評估其資安管理制度、資安專責人員配置與專業能力，以及過往資安事件紀錄與處理能力？', hint: '廠商評選紀錄、資格審查文件、廠商資安能力證明（如 ISO 27001 證書）。確認廠商及專案人員非屬大陸地區廠商或陸籍身分，且不使用危害國家資安產品' },
        { id: '6.3', text: '採購契約中是否已明確訂定資通安全相關條款，包括但不限於：資安責任歸屬、保密義務、禁止轉包或再委託之限制、違約罰則等？', hint: '採購契約書（含資安條款、保密義務、轉包限制等）。是否已明確規範受託廠商不得未經機關同意進行轉包或複委託，且經同意之複委託對象須符合相同資安要求' },
        { id: '6.4', text: '委外開發之資通系統，是否於契約中要求受託者於適當時機（如上線前、重大變更後）進行安全性檢測並提出合格檢測證明？', hint: '安全性檢測報告（弱點掃描、滲透測試、源碼檢測等）。涉及核心資通系統或達一定金額門檻之案件，是否已委託第三方辦理資安檢測' },
        { id: '6.5', text: '契約中是否已訂定資通安全事件之通報程序與時限，確保受託者於發生資安事件時能即時通知本單位？', hint: '契約條文（載明資安事件通報義務及時限，不得逾 1 小時，並配合後續應變處理）' },
        { id: '6.6', text: '委外期間，是否落實對受託者之履約安全管理，包括：定期檢視廠商人員異動、存取權限控管、安全開發生命週期（SSDLC）之執行狀況等？', hint: '履約管理紀錄、廠商人員名單及權限異動紀錄、SSDLC 執行紀錄。明確要求受託廠商於履約期間發生資安事件時須即時通報並負責採取補救與防護措施' },
        { id: '6.7', text: '辦理採購案驗收時，是否確認資安相關需求均已落實，包含安全性檢測報告之審閱、已知弱點之修補確認等？', hint: '驗收紀錄、安全性檢測報告、弱點修補確認文件、系統文件交付清單。確認第三方元件清單及授權狀態，並確認系統文件、設定文件及必要技術資料均已完整交付' },
        { id: '6.8', text: '契約中是否已訂定委託關係終止或解除時，受託者所持有之本案相關資料的返還、移交、刪除或銷毀作法？', hint: '契約條文（載明資料返還、移交、銷毀機制）、資料處理確認書。須具體說明資料格式、交付方式、期限、責任人等' }
      ]
    },
    {
      section: '7. 資安事件通報', items: [
        { id: '7.1', text: '單位人員是否知悉資通安全事件應於 1 小時內至主管機關（教育部）指定平台完成通報，並瞭解本校之應變處理程序？', hint: '資安事件通報流程文件（SOP、應變處理手冊，載明事件等級、通報時限、責任單位）、宣導紀錄（簽到表、教材、照片）、資安事件通報演練紀錄（實際演練流程、通報紀錄、應變回覆報告與改善建議）' }
      ]
    },
    {
