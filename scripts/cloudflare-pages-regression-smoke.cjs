...
async function waitForChecklistListReady(page, timeout = 25000) {
  await page.waitForFunction(() => {
    ...
    const keyword = !!document.querySelector('#cl-list-keyword');
    const shell = !!document.querySelector('.cl-list-shell')
      || !!document.querySelector('.checklist-list-header')
      || !!document.querySelector('[data-checklist-route="list"]');
    return (routeState === 'ready' && keyword) || (keyword && shell);
  }, undefined, { timeout });
}
  ...
  try {
    ...
    for (const spec of PUBLIC_DESKTOP_VISUAL_SPECS) {
      ...
      const result = await compareAgainstBaseline(comparePage, baselinePath, actualPath, {
        maxDiffRatio: publicDesktopDiff,
        sampleScale: getVisualSampleScale(spec, 'desktop')
      });
    ...
    for (const spec of PUBLIC_MOBILE_VISUAL_SPECS) {
      ...
      const result = await compareAgainstBaseline(comparePage, baselinePath, actualPath, {
        maxDiffRatio: publicMobileDiff,
        sampleScale: getVisualSampleScale(spec, 'mobile')
      });
