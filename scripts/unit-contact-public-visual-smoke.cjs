...
async function runPublicRouteChecks(browser, pushStep) {
  ...
  try {
    await verifyPublicRoute(page, '#apply-unit-contact', () => !!document.querySelector('#unit-contact-apply-form'));
    pushStep('unit-contact-public:apply-loaded', true, '申請單位管理員');

    await verifyPublicRoute(page, '#apply-unit-contact-status', () => !!document.querySelector('#unit-contact-status-form'));
    ...
    await verifyPublicRoute(page, '#apply-unit-contact-success/UCA-SMOKE-SUCCESS-001', () => !!document.querySelector('.unit-contact-success-note, .empty-state'));
    pushStep('unit-contact-public:success-loaded', true, 'success route rendered');

    await verifyPublicRoute(page, '#activate-unit-contact/UCA-SMOKE-SUCCESS-001', () => !!document.querySelector('#activate-unit-contact-form, .unit-contact-activate-card, .card'));
