...
function loadCorrectiveActionSchema() {
  const schemaPath = path.join(projectRoot(), 'm365', 'sharepoint', 'corrective-action-lists.schema.json');
...
(async () => {
  try {
    ...
    const schema = loadCorrectiveActionSchema();
    const report = {
      siteId,
      lists: []
    };
