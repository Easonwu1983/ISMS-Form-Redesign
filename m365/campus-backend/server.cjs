...
const dashboardRouter = createDashboardRouter({
  writeJson, requestAuthz, cleanText
});
...
const opsRouter = createOpsRouter({
  ...
  globalRateLimitMaxRequests: GLOBAL_RATE_LIMIT_MAX_REQUESTS,
...
function createServer() {
  return http.createServer(async (req, res) => {
    ...
    try {
      // ── Dashboard & My-Tasks (delegated to dashboard-backend.cjs) ──
      if (await dashboardRouter.tryHandle(req, res, origin, url)) return;
      // ── Ops endpoints (delegated to ops-backend.cjs) ──
      if (await opsRouter.tryHandle(req, res, origin, url)) return;
